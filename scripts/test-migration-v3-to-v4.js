...
const DIRECT_MAPPINGS = {
  ...
  'Hybrid': DIIv4BusinessModel.PLATAFORMA_TECNOLOGICA
};
...
class V3toV4Migrator {
   ...
   */
  classifyPlatformCompany(company, result) {
   ...
   */
  classifyManufacturingCompany(company, result) {
    const industryLower = (company.Real_Industry || '').toLowerCase();
    const employees = parseInt(company.Employee_Count) || 0;
  ...
  validateCompanyData(company) {
    ...
    // Validate DII scores
    const scores = ['AER', 'HFP', 'BRI', 'TRD', 'RRG'];
...
module.exports = { V3toV4Migrator, runMigrationTest };