...
function ModelSelector({ businessModels, onSelect }) {
  const getColorClass = (diiBase) => {
    if (diiBase >= 1.0) return 'border-dii-success hover:bg-green-50';
    if (diiBase >= 0.6) return 'border-dii-warning hover:bg-yellow-50';
    return 'border-dii-danger hover:bg-red-50';
  };
      ...
      {Object.values(businessModels).map((model) => {
        ...
        return (
          ...
          >
            ...
            {/* Hover Indicator */}
            <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-transparent via-dii-secondary to-transparent opacity-0 hover:opacity-100 transition-opacity rounded-b-lg" />
          </div>
        );
      })}
    </div>
