    ...
    >
      ...
      {nextDimensionHint && (
        <motion.div
          ...
          className="border-t bg-gradient-to-r from-blue-50 to-purple-50 p-4"
        >
          <button
    ...
    >
          ...
          {strongDimensions.length > 0 ? (
            <ul className="space-y-1">
              {strongDimensions.map(insight => (
                <li key={insight.dimension} className="text-sm text-gray-600 flex items-center gap-2">
            ...
            <p className="text-sm text-gray-500 italic">No standout strengths yet</p>
          )}
        </div>
      ...
      {currentDII !== undefined && (
        <div className="text-center pt-4 border-t">
          <div className="text-sm text-gray-600 mb-1">Current Immunity Score</div>
          <div className={cn(
            "text-3xl font-bold",
            currentDII >= 70 ? "text-green-600" :
            currentDII >= 40 ? "text-yellow-600" : "text-red-600"
          )}>
            {currentDII}
          </div>
        </div>
